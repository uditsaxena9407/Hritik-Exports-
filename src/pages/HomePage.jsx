...
// Define MotionLink once to avoid the syntax error and apply motion properties to Link
  ...
  return (
    <main className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 pt-24 overflow-hidden">
      ...
      >
        ...
        <div className="relative z-10 flex flex-col lg:flex-row gap-8 lg:gap-12 items-center p-6 sm:p-10 lg:p-20 text-white">
          ...
          >
            ...
            >
              {/* NOTE: If you intended for this button to also navigate with motion effects,
                         replace motion.button with MotionLink as well. */}
              <motion.button
                whileHover={{ scale: 1.06 }}
                whileTap={{ scale: 0.96 }}
                // setPage('products') is redundant if using Link, but kept for legacy setPage prop usage
              ...
              >
                {/* Ensure the Link component is not nested inside a button if the button also has an onClick handler. 
                    It is best practice to move the motion properties to the Link itself. 
                    I'm keeping your original structure here, assuming Link is meant only for navigation. */}
      ...
      <section className="mb-12 sm:mb-24">
        ...
        {/* 👇️ THIS IS THE FINAL, CORRECTED BUTTON BLOCK
            Wrapped in a motion.div for the scroll-in animation (like the one before the error)
            and uses the correct MotionLink definition for navigation and hover effects.
        */}
        <motion.div 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
        ...
        >
            <MotionLink 
                ...
                whileTap={{ scale: 0.95 }}
                className="text-green-700 font-bold text-base sm:text-lg inline-flex items-center gap-3 mx-auto transition-all"
            >
